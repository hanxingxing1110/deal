...
if (Test-PythonCommand "python") {
    & python -m crypto_agent.cli --source sample --paper-trade
}
elseif (Test-PythonCommand "py") {
    & py -3 -m crypto_agent.cli --source sample --paper-trade
}
elseif (Test-Path $bundledPython) {
    & $bundledPython -m crypto_agent.cli --source sample --paper-trade
}
else {
